...
function render() {
  ...
  document.querySelector<HTMLButtonElement>("#settings-create-account")?.addEventListener("click", async () => { try { await api.createAccount(window.prompt("Public display name") ?? ""); selected = ""; page = "settings"; await refresh(); } catch (error) { flash(`Could not create account: ${String(error)}`); } });
